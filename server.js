  ...
  ws.on('message', (message) => {
    ...
    if (data.type === 'setUsername') {
      ...
      if (existingClient) {
        ...
        console.log(`Username "${data.username}" is already taken by another client.`);
        ws.send(JSON.stringify({ type: 'usernameTaken', message: 'Username is already taken. Please choose another.' }));
    ...
    if (data.type === 'challengePlayer') {
      ...
      if (opponent && !opponent.inGame) {
        ...
        const headToHeadStats = getHeadToHeadStats(ws.username, opponent.username);
        ...
        console.log(`Challenge failed: ${data.opponentUsername} is already in a game, not found, or there's an issue.`);
      }
    }
    ...
    if (data.type === 'acceptChallenge') {
      ...
      if (challenger && !challenger.inGame) {
        console.log(`${ws.username} accepted challenge from ${challenger.username}`);
        ws.inGame = true;
        ...
        ws.send(JSON.stringify({ type: 'gameStart', opponent: challenger.username }));
        challenger.send(JSON.stringify({ type: 'gameStart', opponent: ws.username }));
      } else {
        console.log(`Challenge acceptance failed: ${data.challenger} not available.`);
      }
    }
    ...
    if (data.type === 'makeMove') {
      ...
      if (opponent) {
        ...
        console.log(`${ws.username} made move: ${data.move} against ${opponent.username}`);
    
        if (opponent.currentMove) {
          console.log(`Both players have made their move: ${ws.username} (${ws.currentMove}) vs ${opponent.username} (${opponent.currentMove})`);
          ...
          ws.send(JSON.stringify({ type: 'gameResult', result: result[0], opponentMove: opponent.currentMove }));
          opponent.send(JSON.stringify({ type: 'gameResult', result: result[1], opponentMove: ws.currentMove }));
    
          // Reset moves for both players
          ws.currentMove = null;
          opponent.currentMove = null;
        } else {
          console.log(`Waiting for opponent ${opponent.username} to make a move.`);
        }
      } else {
        console.log(`Error: Opponent not found for ${ws.username}. Opponent username: ${data.opponent}`);
      }
    }
    ...
    if (data.type === 'requestRematch') {
      ...
      if (opponent && opponent.inGame) {
        console.log(`${ws.username} requested a rematch with ${opponent.username}`);
        ...
        console.log(`Rematch started between ${ws.username} and ${opponent.username}`);
      } else {
...
function broadcastAvailablePlayers() {
  ...
  availablePlayers.forEach(player => {
    player.send(JSON.stringify({ type: 'availablePlayers', players: playerList }));
  });
}
...
function getStatsForPlayer(username) {
  ...
  return playerStats[username] || { wins: 0, losses: 0, draws: 0, gamesPlayed: 0 };
}
...
function getHeadToHeadStats(player1, player2) {
  ...
  return headToHeadStats[key] || { wins: 0, losses: 0, draws: 0, gamesPlayed: 0 };
}
